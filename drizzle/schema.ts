...
/**
 * Operator messages — one-off personal notes queued by an admin (Vos Arkana)
 * for a specific user. Delivered by ORIEL at the start of that user's next
 * chat turn, then marked delivered so they never repeat.
 */
export const operatorMessages = mysqlTable(
  "operatorMessages",
  {
    id: int("id").autoincrement().primaryKey(),
    targetUserId: int("targetUserId").notNull(),
    senderLabel: varchar("senderLabel", { length: 64 })
      .default("Vos Arkana")
      .notNull(),
  ...
  table => [
    index("idx_operator_messages_target_delivered").on(
      table.targetUserId,
      table.delivered
    ),
  ]
...
export type InsertOperatorMessage = typeof operatorMessages.$inferInsert;

/**
 * Operator replies — the ONLY thing captured going the other direction
 * (user -> admin). A user must explicitly say "REPLY <message>"; nothing
 * else from their conversation with ORIEL is ever stored here or exposed
 * to the admin. Privacy-by-design: exact opt-in text only, never a window
 * into the rest of their chat history.
 */
export const operatorReplies = mysqlTable(
  "operatorReplies",
  {
    id: int("id").autoincrement().primaryKey(),
  ...
  table => [
    index("idx_operator_replies_from_read").on(table.fromUserId, table.read),
  ]
);

export type OperatorReply = typeof operatorReplies.$inferSelect;
export type InsertOperatorReply = typeof operatorReplies.$inferInsert;
