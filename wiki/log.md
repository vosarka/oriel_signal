...
- **Mistral Voxtral leads the TTS chain.** $0.016 per 1000 characters against ElevenLabs at $0.05 for Flash and $0.10 for Multilingual: 1.4 cents on a 900-character reply instead of 9. It also bills the account that already pays for ORIEL's thinking, so there is one balance to watch rather than three. ElevenLabs stays behind it.
...
- *Redaction is not truncation.* `describeFailure` capped the provider's response body at 300 characters and called that a privacy measure. An echoed payload sits in the first 300 characters as easily as past them, so a private memory could reach the log intact. AGENTS.md rule 3 says never log secrets, and a sentence somebody told ORIEL in confidence is one. The content we sent is now removed from the body by name before anything is capped or logged.
