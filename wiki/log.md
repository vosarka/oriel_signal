...
## [2026-09-12] fix | The voice branch took ORIEL down on the first boot after merge
- Production crashed on start with `ERR_UNSUPPORTED_DIR_IMPORT` for `@mistralai/mistralai/extra/realtime`. The package ships no `exports` map, so Node's ESM resolver requires a subpath to name a file; the specifier named a directory. One character short of working: `.../extra/realtime/index.js`.
- **Three layers of checking looked at this import and none of them could see it.** Vitest resolves through Vite, a bundler, which accepts a directory specifier by finding index.js inside it. `tsc` resolves types, and the `.d.ts` sits in that directory, so it was satisfied. esbuild bundles with `--packages=external`, so it never resolves the specifier at all. Everything was green and the process died on the first boot.
- The tests around the transcription helpers were thorough and irrelevant here: coverage of what a module does says nothing about whether the module can be loaded.
- **What now guards it:** `server/_core/node-module-resolution.test.ts` reads the server sources, and for every bare subpath import into a dependency with no `exports` map, fails if the target is a directory. Mutation-checked by putting the broken specifier back, which turns it red with the corrected specifier in the message.
- **What actually proved the fix:** building the bundle and running `node dist/index.js` the way Railway does. The crash moved from module resolution to a missing `BETTER_AUTH_SECRET`, and with dummy secrets the server reached `Server running on`, printing `[LLM][config]`, `[Memory][config]`, `[TTS][config]` and both WebSocket proxies. No amount of unit testing would have said that.
- Lesson worth keeping: a deploy that only ever runs `pnpm run build` proves the bundle can be written, not that it can be started.

## [2026-09-12] chore | Measuring why MindMemOS is slow, before guessing at a fix
- Production falls back to TiDB on `MindMemOS request timed out after 5000ms`, so ORIEL answers with memories chosen by importance rather than by relevance to what was just said. The recall that makes it feel like it remembers a person is effectively off, and nothing in the interface shows it.
- **A timeout message reports our own clock, not the service.** "Too slow" hides two problems with opposite fixes: a service answering just past the deadline wants a longer deadline, and a service that never answers wants investigating. Raising the limit on the second one only makes every turn slower for nothing.
- One thing the existing log already settles: the abort lands at exactly the deadline, so the connection is accepted and unanswered. A refused connection or a DNS failure throws with a different name, well before five seconds. Reachable and slow, not down.
- Also found in the code rather than the log: **a turn fires two searches, not one** - one on the person's words, one on the same words behind ORIEL's working-view prefix - and `Promise.all` means either one timing out drops the whole turn to the fallback.
- **Two of the three review findings were holes in the instrument itself, which is the worst place for them.** The timing stopped when the response headers arrived, so a service answering headers at once and dribbling the body out for six seconds would have been filed as fast - and the abort cannot see that either, because it fires on headers and nothing after. And every transport failure reads as `TypeError: fetch failed` in Node with the real reason tucked into `error.cause.code`, so a refused connection, an unknown host and a rejected certificate all printed the same word. A diagnostic that cannot tell those apart is worse than none, because it is believed. Body reads are timed separately now and the cause code is printed; the clock is monotonic rather than the wall clock.
- **The fourth round caught the same mistake a second time, one commit after the first.** Wrapping `describeFailure` in the body timer looked like coverage and was not: that function catches every read failure and always returns a string, so the timer's failure branch was unreachable and an abandoned read at the two-second deadline logged as a completed one. I had even claimed on the thread that a good body and a bad one could no longer be confused. They could. The timing moved inside `describeFailure`, the only scope that knows which happened, and a test drives a body that throws to prove the abandoned branch is reachable.
- **A third round found the same mistake I had been criticising all day.** The logging helper grew an `extra` parameter that no caller ever passed, so it rendered as nothing on every line: a fix that never runs, reading like protection. Gone. The other two were real gaps in coverage rather than in shape - error bodies were read outside the timed region, so only successful calls were measured and a slow failure would have looked instant, and the body line carried no query size, so body slowness could not be set beside the one suspect the header line records.
- `[MindMemOS][timing]` now prints per call: which call it was (`add`, `search_user`, `search_view`), the elapsed milliseconds, the outcome (an HTTP status, our own `timeout_at_5000ms`, or a named transport failure), and the query length. The query itself is never logged; its size is a fair suspect for the slowness and carries nothing private (AGENTS.md rule 3).
- Vos, asked whether he would rather ORIEL waited for the right memories or answered faster with lesser ones: more apt and more correct. So the fix will lean toward waiting - but only once the numbers say waiting would help.
- Verification: 992 passed, 5 failed, the same five that fail on main. Built the bundle and booted `node dist/index.js` as Railway does; the server reached `Server running on`.

## [2026-09-12] fix | Every memory write had been refused, and the timing said which question to ask
- The measurement from earlier today answered both questions on the first deploy, and the answers were different from each other.
- **Writes were never slow. They were refused.** `[MindMemOS][timing] add headers_http_422 elapsed_ms=903`, then the body in a millisecond, then `gateway.invalid_request`: "memory request does not match the current API schema". Eight refusals in ten minutes of conversation. The semantic index has been receiving nothing, which means a working search would have found nothing new to return either.
- **The cause is in the vendor's own SDK.** Their documented call is `client.memory.add(messages=[...], mode="sync")`. We were sending `async_mode: "sync"` **and** `mode: "fine"`: the field was renamed, two fields collapsed into one, and "fine" belonged to the older field's vocabulary. Sending both, with the wrong value in the surviving one, is what the gateway rejected.
- The response parsing needed nothing: `memory_id` inside `memories` is still the documented shape, and `parseCloudId` already reads it.
- **Searches are a different failure and stay untouched.** They never answer within five seconds while `add` on the same host answers in half a second, so the service is up and one endpoint does not respond. Query length is ruled out as a suspect: 52 and 72 characters.
- **A privacy worry that turned out not to be one.** The vendor's HTTP example for search omits `user_id`, and matching it blindly could have let ORIEL recall another person's memories. Their docs settle it: `user_id` "can be overridden in an individual add or search call". Our search payload is valid; the hang is server-side.
- Verification: 995 passed, 5 failed, the same five that fail on main. Built the bundle and booted `node dist/index.js` as Railway does.

## [2026-09-13] change | ORIEL does not speak until somebody asks it to
- The voice used to start on its own: `voicePreference` defaulted to `sophianic` for every new account and every fresh browser. Vos asked for the opposite, and the reason is not preference but consent. A reply that begins speaking decides, on the person's behalf, that the room they are in is one where they can be spoken to aloud. It might be a shared office, a bus, someone asleep next door.
- **The default lived in several places that all had to agree**, which is why a test now checks them rather than trusting them: the column default in `drizzle/schema.ts` for new accounts, the migration steps in `server/db.ts` that set that column, the initial state in `Conduit.tsx` before a stored preference loads, and the fallback used when a stored value cannot be recognised. Any one left as a voice would have put the sound back for some group of people.
- The catch-all migration that normalised unknown values to `sophianic` now normalises them to `none`. Guessing wrong toward speech is the louder mistake.
- **Existing accounts are switched to silence too, once.** Put the choice to Vos - leave them or switch them all off - and he chose to switch them all off and let people pick again. The column defaulted to a voice from the day it existed, so a person who never touched the selector is indistinguishable in the data from one who chose; the only way to reach the ones who were never asked was to include the ones who were. It is one filtered `UPDATE` and nothing else: no `DELETE`, no `DROP`, no message touched.
- **Exactly once was the whole difficulty.** The other migration steps are written to be harmless when repeated - adding a column that exists, widening an enum already wide. A data change has no such property. "Set every voice preference to silence" is right the first time and wrong the second, because by then people have chosen. So a small `appliedOneOffMigrations` ledger gates it: the insert claims the work, only the run that wins the claim performs the change, and every later run finds the name taken. The claim is released only when the statement demonstrably failed, so a crash between the two leaves the change marked as done rather than repeating it - the safe way to be wrong when the alternative is overriding somebody's explicit choice.

### What the review round corrected, and what I had told Vos wrongly
- **I said to watch the first deploy for `[Migrations] Reset every voice preference to silence (one-off)`. That line will never appear there.** Codex caught it and the boot log proves it: this deployment prints `[Migrations] Skipped — RUN_MIGRATIONS is disabled` and returns before any step runs. The reset is not dead code - `pnpm db:migrate` sets the flag itself, which is how every schema change in this repo actually reaches the database - but it is a deliberate act by Vos, not something a deploy does for him. Telling him to watch a log line for it was telling him to wait for something that was never coming.
- **A stored voice could still speak while the account was still loading.** The page read `localStorage` on mount and the account's own preference in a second effect, and sending was enabled in between, so a message sent in that window could be spoken by a preference the account had not confirmed. Both effects are now one, it waits for auth to settle, and where it reads from follows where the selector writes to: signed in, the account holds the preference; signed out, this browser does. The two had never agreed - the selector has always written to the account when signed in and to `localStorage` only when not.
- **One of my own tests could not fail.** It searched for ``SET `voicePreference` = 'none'`` with plain backticks; `db.ts` writes its SQL inside template literals, where every backtick is escaped, so the filter matched nothing and the assertion passed on an empty list. Same shape as the dead `extra` parameter and the unreachable catch branch from the MindMemOS rounds - a check that reads like protection and runs like nothing. The suite now unescapes first, has a test whose only job is to fail if that unescaping stops matching how `db.ts` writes SQL, and every one of the eight was mutation-checked: nine deliberate breakages, nine reds.
- **Prettier churn again, in `drizzle/schema.ts` and `server/db.ts`.** Third time this stretch. Reformatting five untouched regions of `db.ts` hides ten real lines inside eighty-six and makes a reviewer read the whole file to find the change. Reverted by hand, hunk by hunk, and the diff checked line by line before pushing. AGENTS.md rule 6.
- **The replacement test had two holes of its own, and the review found one of them.** Its containment check resolved each blanket `UPDATE` to the nearest call marker in the preceding 500 characters — and when it recognised nothing, it defaulted to "this is the gated one". A reset dropped into the ordinary steps array has no marker near it, so the single most likely way to make this mistake was the one shape waved through. It classifies positively now: find the gate call's span by matching parentheses, and flag every blanket reset outside it. Chasing that down surfaced the second hole, which nothing had reported: the "allow the catch-all" filter looked ahead a fixed 200 characters and so borrowed the *neighbouring* statement's `NOT IN`, excusing a reset inserted next to it. It asks the statement's own line now.
- **Three rounds, three versions of the same test, two of them unable to catch what they were for.** The lesson is not "mutation-check the test" — I did, and the mutation I chose was one the test already caught. It is that a guard needs breaking in the shape of the mistake somebody would actually make, not the shape that is convenient to write. Thirteen mutations now, including a reset in the steps array and one adjacent to the catch-all.
- Verification: 939 passed, 3 failed, and 11 files that fail to load for want of a `DATABASE_URL` - identical to a clean checkout of `main` in this container. `tsc` reports the one pre-existing `routers.ts` error and nothing else. Built the bundle and booted `node dist/index.js` as Railway does; it reached `Server running on` and stayed up. Booted it a second time with `RUN_MIGRATIONS=true` against an unreachable database to watch the gate fail safely: the claim errors, it logs and returns, and no preference is touched.

## [2026-09-23] feat | The daily signal is persisted, scheduled, and shown in the archive carrier
- The clarity engine, register rules and generation prompt were already on this branch (`shared/daily-signal.ts`, `shared/daily-signal-prompt.ts`, `daily_signals` created by `scripts/create-daily-signals-table.ts`, not drizzle-kit, because the journal has drifted from production). This entry covers wiring them in.
- `server/daily-signal-service.ts`: `getTodaysSignal()` is a plain read and never generates, so no visitor waits on a model call. `getOrCreateTodaysSignal()` reads or generates, checks the body's shape for its register (3-4 shards when FRACTURED, opening/middle/closing otherwise, title and archetype always), retries once on a malformed reply and otherwise writes nothing. The insert leans on `signalDate`'s UNIQUE key (`ON DUPLICATE KEY UPDATE id = id`) instead of check-then-insert.
- `archive.dailySignal.today` (public, read-only). The project has no scheduler, so `server/_core/index.ts` polls every 5 minutes; once today's row exists each poll is one SELECT. After 6 consecutive failures in a UTC day it stops until tomorrow instead of calling a dead provider all day. Vos decided a bad day stays public: no regeneration path.
- `TransmissionCarrier` renders either an archive transmission or today's signal in the same slots. The signal's own clarity register replaces the archive's VTIP lookup (different axes), its body lines sit behind the same carrierlock, and the fragment link is hidden because a signal has no detail page. `/archive` shows today's signal when there is one, TX-080 otherwise, and refetches so a page open across UTC midnight catches up.
- **The daily test is not written by the model.** Baseline runs produced claims that could not fail ("your shadow will not move when you step out of light"). Three prompt revisions traded that for banned verdict clauses, durations, and finally wrong physics. A fixed bank of single sentences worked, but Vos did not want it to repeat. The test is now an experiment: an action that really turns out more than one way, with two of its results shown each day, each saying what it means (`EXPERIMENTS`, `falsifierFor`). Ships with 6 experiments, which repeat after about 18 days; a larger bank, reviewed by Vos, is the follow-up.
- Verified against the running app: the scheduler wrote a real row to production TiDB, the tRPC read returned it, and Vos confirmed `/archive` renders it.

### What the review round corrected
- An action said "Clap once", against the bank's own no-counts rule; the test only looked for digits. Reworded, and the test now rejects count words in actions. (cubic)
- The test named "accepts three or four shards" only checked three. (cubic)
- The page never refetched today's signal. Now every 5 minutes, and the carrier is keyed by id so a new signal arriving while the page is open comes in sealed, not already unlocked. (cubic)
- Flagged by review, left for separate work because they predate this entry: the daily phenomena trio repeats a phenomenon on about 3 days in 17, contrary to its comment; the DDL types (`DATE`, `JSON`) differ from the Drizzle schema (`varchar`, `text`), harmless against the live table; the table is not created by any deploy step.
## [2026-09-23] fix | The memory fallback no longer ignores what the person just said
- Vos noticed that when a chat opened with an elaborate message (mathematics, physics, birds), every later reply dragged those same themes back in, whatever the conversation turned to. Cause: whenever MindMemOS did not fill the turn, `selectMemoriesForTurn` topped up from `getRelevantMemories`, which ordered strictly by `importance DESC, lastAccessed DESC` and never looked at the current message - then refreshed `lastAccessed` on every row it returned. A memory that scored high once kept winning, and kept being refreshed. `logResolvedMemoryConfig` already described the symptom ("the same few every turn regardless of subject"); nothing had fixed it.
- `getRelevantMemories(userId, limit, userMessage?)`: with a message it is a candidate provider for one turn - it scans up to 200 active rows and ranks those sharing any keyword with the message ahead of those that do not (`rankMemoriesByRelevance`, stable partition, existing order kept inside each group). It no longer marks anything; `selectMemoriesForTurn` marks only the fallback rows that survive `composeTurnMemories` and reach the model. Without a message (the two post-turn snapshot calls in `processConversationMemory`) query, cost and marking are unchanged.
- `extractContextKeywords` moved from `oriel-transmission-mode.ts` into dependency-free `server/context-keywords.ts`.

### What the review round corrected
- **An import cycle I had been told did not exist.** Importing the keyword helper from `oriel-transmission-mode` closed a loop: `oriel-memory-retrieval` -> `oriel-transmission-mode` -> `oriel-prompt-context` -> `oriel-context-layers` -> `oriel-memory-retrieval`. The exploration I relied on checked only direct imports back into the memory modules. Tests passed through it; it is gone now rather than tolerated. (cubic)
- **The first version still refreshed rows the model never saw.** It marked every fallback row it returned, before composition dropped some of them - a smaller copy of the loop this change exists to break. Now marked after final selection, and a test pins it: rows dropped by composition and rows that came from search are not marked. An injected fallback never marks real rows, so no test can touch the database. (cubic, twice)
- **Relevance was only checked against each text's top twelve keywords**, so a match on a later word in a long message was lost. Every keyword counts now; a test covers the thirteenth. (cubic)
- **The candidate scan was capped at 32 rows.** A long-standing user's on-topic memory below that importance cut could never be ranked. Raised to 200 in the same single query; a person with more active memories than that can still miss one, marked `ponytail:` in the code with a content-aware query as the upgrade path. (Codex, cubic)
...
- **Renamed TX-GEN to DFS (Daily Field Signal)**, as Vos asked; new signals are `DFS-690007` onward. The four rows already in production are renamed by `scripts/rename-daily-signal-ids.ts` (dry run by default, `--apply` to write, idempotent, only `txGenId` changes). The column keeps its `txGenId` name. `oriel-transmission-mode.ts` still uses `TX-GEN-` for its own generated transmissions; that is a different feature and was left alone.
- **Each signal has a link now**: `/archive?dfs=690006` opens the archive with that signal seated and the DFS tab open, and choosing a row writes its link into the address bar. It matches on the serial, not the prefix, so it works for rows written either side of the rename. Verified on the running app: `?dfs=690005` opens 22 September sealed; choosing today rewrites the address to `?dfs=690006`.
- **A feed for the community bot**: `GET /api/daily-signal/today` returns today's signal as JSON with a ready `post` field (id, title, clarity, the carrier line, the `?dfs=` link) and 404 until the day's signal exists. Vos's existing bot, already admin in the community group, fetches it once a day and posts `post` verbatim; nothing on this side posts anywhere. The post is a teaser - the body stays sealed on the site. The link's host comes from `APP_BASE_URL`, falling back to `https://orielsignal.space`.
- **The key line and the three traps**, taken from Vos's chat prompt into the site generator. The model now also writes `key`: one lowercase line of seven words or fewer, placed in the body after the voice and before the archetype; more than seven is rejected. The prompt names the motivational, esoteric and generic traps. The test and the format stay set by code: a chat run of the same prompt gave the tuning-fork test backwards and invented "molecules aligned before glass shatters", which is why neither is left to the model on the site. Read-only runs against the real model: 6 of 8 valid, every failure an eight-word key; the retry and the scheduler's repeated attempts absorb that at the cost of an extra call on some days. Watch: FRACTURED keys lean on "listen for/to the...".

## [2026-09-23] fix | The archive stopped jumping under the pointer, and only today is Live
- Vos reported that before the carrierlock opened, the whole page bounced about a centimetre up and down, making things hard to click. Cause: the sealed body redraws its cipher every 220ms, and the cipher glyphs are wider and taller than the letters they replace, so each paragraph reflowed and changed height every tick and everything below moved with it. The daily signal's seven-line body made it far worse than the old single paragraph. Each paragraph now holds its plain text invisibly for layout and draws the cipher absolutely on top, so the cipher can never move the page.
- Measured headless in Chrome, sampling the section below the carrier every 100ms: the live site moved 32px back and forth; with the fix 0px while sealed and 0px through the decrypt. After opening, all seven paragraphs show text identical to the plain lines.
- The carrier's rail said "Live" on every carrier. It now says "Live" only for today's signal and "Recorded" (no pulse) for a past day or a recovered transmission.
- Also done today after #19 merged: `scripts/rename-daily-signal-ids.ts --apply` renamed the four production rows to DFS (4 renamed, row count unchanged, none left on TX-GEN), and the live feed at `/api/daily-signal/today` answers with the real domain.

## [2026-09-23] fix | Closing the review leftovers from the daily signal work
- **The day's phenomena could repeat.** The rotation used strides 1, 5 and 11 on a 17-item list and its comment said being coprime meant no collisions; it collided on 3 days in every 17, 19 September among them, so the prompt offered the same image twice. Now fixed offsets 0, +6, +12, which cannot collide, with a test over 60 days. The first phenomenon of each day is unchanged.
- **The table script and the schema disagreed without saying why.** Production's `daily_signals` has `signalDate DATE` and `bodyLines JSON`; `drizzle/schema.ts` declares varchar and text. That is deliberate and works: drizzle asks mysql2 for DATE as a string, and the insert passes an already-stringified array that MySQL parses. Both files now say so, so nobody "aligns" `bodyLines` to `json()` alone and double-encodes it. The script's rollback note ("DROP TABLE, nothing references it") was false once the scheduler, archive and bot feed read the table; replaced, and it now says no deploy creates the table, so a fresh database needs it run once by hand.
- **The FRACTURED example contradicted its own rule**: three resolved sentences and a direct "You" under a rule asking for unresolved fragments without finite verbs. Replaced with fragments, marked as register only.
- **Archive details**: the ALL REGISTERS cell showed four lit bits even when empty; only the prime marker in `IIII′II` is coloured now, not the whole suffix; removed the unused `.tx-headline__mark` rule; the sealed cipher follows the reduced-motion setting live instead of sampling it once, so turning it on stops the animation.
- Also confirmed after #20 deployed: the live archive moves 0px while sealed (was 32px) and the rail reads Live on today, Recorded on a past day.

## [2026-09-24] feat | ΩX Oracle Stream: oracles arrive on their own, and wait for approval
- Vos asked for oracles to appear "at random, when a threshold is reached", and chose to approve each one before it goes public. The first, ΩX-001, is drafted by hand; from then on the stream drafts them itself.
- **When**: `shared/oracle-stream.ts` runs a second wave, "pressure", beside the daily clarity wave. An oracle day is a rising crossing of 18 with no other crossing in the 5 days before. Stateless and reproducible: about 13 a year, never closer than 6 days, never more than 7 weeks apart. First automatic days: 2026-10-01, 11-02, 12-10, 2027-01-12.
- **What**: the model never picks the subject. Code draws the domain, mode of change, Law of One seed (unattributed) and a motif word from the oracle number, with coprime strides so consecutive oracles never share any of them. The model fills JSON slots; the fixed ΩX caption text (template, clarity lines, channel statuses, hashtags) is assembled by code. Each part gets its own clarity: Past below 64, Present 64 to 84, Future 96 and above.
- **Validation**, one rule per failure the earlier read-only eval produced: literal `{braces}`, an inflection that repeats the Past post instead of answering it, a motif missing from a part. Also rejected: any digit (where invented dates and numbers live) and the stale words. Lists one line too long are cut rather than retried (mistral-large regularly gives four outcomes).
- **Release**: no schema change. The scheduler writes three `Draft` rows on an oracle day, at most three failed attempts a day. **Publish** in `/admin` (on the Past row) confirms Past; the scheduler confirms Present the next UTC day and Future the day after. Every public oracle read (`getAllOracles`, by id, by thread, threads, and ORIEL's transmission-mode references) now returns only Confirmed/Prophetic rows; admin reads through `getAllOraclesForAdmin`. `ponytail:` "released" is read from `updatedAt`, so editing a released part delays the next by a day.
- `scripts/generate-oracle-draft.ts --out f.json` generates and prints the next oracle without writing; `--insert f.json` stores that exact text as Draft. The preview of ΩX-001 passed validation first time on mistral-large-latest.
- `OracleDetail` renders the caption line by line (`pre-line`). Palette untouched; that waits on Vos's canon decision.
- Verification: 10 new tests (trigger dates, draw non-repetition, clarity bands, the four rejections, caption assembly); the full suite's 5 failures are the same 5 as on `main`. `tsc` reports only the pre-existing `routers.ts` error.

## [2026-09-24] feat | The book page shows the new 64-page book
- Vos asked to redo `/tetradic-signature` with the correct data and new images. The book is now 64 pages in three parts (STRUCTURA_CARTE_V2): I · The System, a ten-chapter manual (pp. 9–28); II · The Record, the same ten chapters with the receiver's numbers plus the central contradiction (pp. 31–52); III · What I Saw, the founder's voice (pp. 55–62). The page still said 12 tetrads · 48 pages.
- Images come from the finished Canva book (`Tetradic Signature Cover.pdf`), **Part I only**: it holds no receiver data. Vos asked that nothing of Gail's appear; the cover, the title pages and all of Parts II–III carry her name, dates or codons, and the Part I chapter 4 page cites `RC29-B` as its example, so none of those are used. Ten chapter plates and the chapter-one text page, as WebP (13–71 KB each) under `public/assets/tetradic-signature/book-v2/`.
- The spread now shows page 09 (plate) and page 10 (Two Moments); the twelve tetrad cards became ten chapter cards with their plates, in two rows of five on desktop and a swipe row on mobile. Register reads "3 parts · 64 pages"; the order status says 64 pages. Price and delivery unchanged. The opening scroll films are untouched.
- Tests updated to the new book, including one that fails if "Gail" or "1961" ever reaches the page markup. The one remaining failure in `tetradic-founder-edition-page.test.ts` (hard-coded colours in the CSS) also fails on `main`.
- **Revised the same day, at Vos's direction:** the obsidian plates are the ChatGPT renders of Part I (21 September, chapters 1–10), not the Canva plates in the PDF; the ivory pages stay from the PDF. Six of each: chapter 1 as the large spread, chapters 2, 3, 7, 8 and 10 as small spreads in the grid, plus a sealed card for Parts II–III (pages 29–64). Three renders in `founder moloz` are Part II (Your Two Moments, Your Descent, Your Field) and show Gail's dates, sun longitudes and codons; they are not used.
- **The film opening is gone.** Vos no longer liked it, and the book opening in the films was not the book people receive. `/tetradic-signature` now opens directly on the Founder Edition: the chapter-one obsidian plate on the left; title, price, "3 parts · 64 pages", delivery and a **Begin your reading · €81,32** button on the right, all on the first screen at 1440×860 (button at 646px). Stacked on mobile. The plate stands in until there is a cover without Gail's name.
- Deleted with it: `TetradicSacredExperience`, its scrub hook, scroll config and CSS, its test suite, and the two films only it used (`04book_opens`, `05page_zoom`, desktop and mobile). The page no longer loads Lenis or the GSAP ticker bridge. Left alone: the older unrouted variants (Video, V2, Simple, Editorial) and the three intro films the Video variant still references.
- The hero button is not in the GSAP intro stagger: animated with the rest it stayed at opacity 0 (it sits after the stagger and carries its own CSS transition). Checkout test updated to "64-page edition".

## [2026-09-25] feat | The daily signal unpacks the Codon of the day
- Vos's idea: the daily signal should be the unpacking of one micro-correction, with a Codon assigned to each day by calculation. The Codon library already holds a micro-correction for all 256 facets (64 Codons × 4), each with its shadow manifestation.
- **Which Codon:** the Moon's tropical longitude at 12:00 UTC (Swiss Ephemeris, `bodyLongitudeAtUtc` added to `ephemeris-service`), mapped through the Mandala with the existing `longitudeToCodonFacet`. The Moon crosses more than one Codon a day, so consecutive days never share one and a lunar month covers most of the wheel; the whole UTC day, and everyone on Earth, gets the same Codon. Chosen over the Sun (about six days per Codon) and over a plain 256-day counter (no link to the sky). `server/daily-codon.ts`.
- **What changes:** the telemetry "Field" is now the Codon ("RC22 · THE GRACE · Transpersonal"); the prompt carries the Codon, its gift and shadow, and the facet description, and asks for the transmission to unpack the day's correction without restating it. The shadow and correction are printed after the archetype **verbatim from the library**, never written by the model. The feed post adds a "Codon of the day" line. The clarity waves, registers, key line and final instruction are unchanged.
- No schema change: the Codon goes in `field`, the correction in `falsifier`. Past signals keep what they were written with.
- Canon cleanup for public posting (`publicText`): cross-references like "(Check RC27/RC3)" and markdown asterisks are stripped; the words stay verbatim.
- Read-only run against mistral-large for 25–28 September: four different Codons (RC22, RC17, RC51, RC27), the voice tracks the facet (the Caretaker day writes about hunger, the Scope day about the gaze). Two of four were rejected by the existing validator (five shards; an eight-word key); retries absorb that.
- **Only the correction is printed.** 38 of the 512 shadow/correction texts touch depression, eating, death, addiction, trauma, abuse or headaches; printed daily to everyone, a shadow line reads as a diagnosis. Vos chose the correction alone; the shadow stays in the prompt as context the model must never name.
- **Two kinds of day, Vos's call:** each day is either classic (rotating field and the physical test, as before) or a Codon day, picked by a hash of the day number so it is reproducible and does not simply alternate (`isCodonDay`, about half each). The experiment bank and field list are therefore back. Upcoming: 25 and 26 September Codon, 27–29 classic, 30 Codon. A read-only run of one of each on mistral-large came back valid; the Codon day prints no shadow.
- Watch: openings keep returning to "Grief is not a weight…" across days and both kinds.
- Stacked on #21 (same files). Tests: 13 across `daily-codon` and `daily-signal`.

## [2026-09-26] fix | DFS titles stop being "The X That Y the Z"
- Vos noticed every daily title had one shape: all seven in production so far ("The Resonance That Shapes the Hollow", "The Bowl That Holds the Sky", "The Hum That Aligns the Bones", …). The prompt caused it: "a noun, an action, and a sacred or natural element", with "The Song That Built the Stones" as the example.
- The frame now sets the day's title form from eight (`TITLE_FORMS`: two nouns with "and", adjective and noun, a moment in time, a place where something waits, a short command, an "Of" phrase, a thing in an unexpected place, a gerund phrase), stride 3 so neighbouring days differ. The validator rejects a title starting "The … That …". Read-only run of eight days on mistral-large: "Where the Coin Hums Its Name", "A Stone in Still Air", "Feel the Floor First", "Holding the Shape of Echo", "Of Walls That Listen"…; one of eight drifted from its form.
- **Found on the way, older than this work:** `initEphemeris` cached the SwissEph object before its WASM had loaded, so two concurrent first calls crashed the second ("reading 'ccall'"). The daily signal calls it one at a time, but two birth-chart calculations right after a restart could hit it. It now caches the init promise and clears it on failure; a cold-start concurrency test fails on the old code and passes on the new.
- Also today: ΩX-001 "Thread Count Echo" inserted as three Draft rows after #22 deployed, rebuilt field for field from the approved preview (the saved JSON had been cleared from temp) and re-validated; the public oracle list still returns none until Vos publishes it.
- **The opening now shows the cover** (`book-v2/cover.webp`, 1100px, 223 KB) in place of the chapter-one plate: Vos's cover with "{ YOUR NAME }" in place of a receiver's name, so it shows the object without anyone's data. Checked at desktop and mobile widths.
