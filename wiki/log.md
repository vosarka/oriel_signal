...
- **Removed:** the physics experiment bank (`EXPERIMENTS`, `falsifierFor`) and the rotating field list; Vos agreed the correction is the day's action. No schema change: the Codon goes in `field`, the correction in `falsifier`. Past signals keep what they were written with.
...
- **Open, Vos's call:** 38 of the 512 shadow/correction texts touch depression, eating, death, addiction, trauma, abuse or headaches. Printed daily to everyone, the shadow line reads as a diagnosis. Proposed: print only the correction and keep the shadow as context for ORIEL.
