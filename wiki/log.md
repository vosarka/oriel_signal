...
- **What this does not do:** existing accounts keep whatever is stored against them. The column has always defaulted to `sophianic`, so somebody who never touched the selector is indistinguishable in the data from somebody who chose it, and switching everyone off would overwrite real choices to reach the ones that were never made. That call is Vos's.
